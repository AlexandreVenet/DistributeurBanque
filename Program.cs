...
namespace DotNet_BorneBanque
{
	...
	internal class Program
	{
		...
		// Regex à tester
		private static Regex _rgxAccount = new Regex(@"^[0-9]{4}$");

		// Regex à tester
		private static Regex _rgxPin = new Regex(@"^[0-9]{3}$");
	...
	6 : quitter
			...
			while(!isChoiceOk)
			{
				...
				_str = Console.ReadLine();
				if(int.TryParse(_str,out result) && result >= 1 && result <= (int) AppState.Quit)
				{
					isChoiceOk = true;
					//break;
				}
		...
		private static void WithDraw()
		{
			...
			Console.WriteLine($"Vous avez retiré {result}. Il reste {_currentAccount.m_balance} sur le compte. Appuyer sur Entrée pour revenir au menu.");
			ControlBackMenu();
		}
		...
		private static void Credit()
		{
			...
			Console.WriteLine($"Vous avez crédité {result}. Votre compte est de {_currentAccount.m_balance}. Appuyer sur Entrée pour revenir au menu.");
			ControlBackMenu();
		}
